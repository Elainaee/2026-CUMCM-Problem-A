...
\textbf{针对问题一}，因长径比 $L/R=12.5$，且时长仅为半小时，轴向端部效应可忽略。我们首先将药材视为无限长圆柱，建立径向一维非稳态传热--传质模型：温度场由 Fourier 定律描述，水分场由
Fick 第二定律描述，中心轴取零通量条件，表面取 Robin 型对流换热与对流传质条件；烘房时变环境
取自附件1，并经 \textbf{Savitzky--Golay 保形滤波}平滑。然后在空间上以有限体积法离散、时间上以 \textbf{Crank--Nicolson} 格式推进数值求解。
...
\textbf{针对问题二}，烘干过程长达 $2\sim3$ 天，端部效应此时不可忽略，我们直接对真实圆柱建立模型。由于预热和恒温干燥阶段参数不同，我们先结合滑动均温与升温斜率双条件，得到预热与恒温两阶段分界时刻  $\mathbf{t_c=6780\ s=1.8833\ h}$，然后采用\textbf{后向欧拉}格式配合 \textbf{Picard 迭代}与\textbf{共轭梯度法}进行数值求解。结果表明
...
\textbf{针对问题三}，因含水率沿径向由外向内递增，中截面中心 $(r,z)=(0,L/2)$ 处最高且下降
最慢，故“各处水分浓度均低于 $0.15\ \mathrm{kg/kg}$”等价于该点达标。在二维轴对称模型上采用
\textbf{自适应变阶 BDF} 算法进行数值求解（与问题四同一算法，$\mathrm{rtol}=2\times10^{-7}$，
步长由局部误差估计自动选取），并以守恒型有限体积格式保证水分守恒；当全域最大含水率
严格低于阈值时，取交叉时刻之后的下一个整秒，得
$\mathbf{t_{\mathrm{dry}}=56.6517\ h}$，此时全域最大含水率为
$\mathbf{0.1499998\ kg/kg}$（四位小数显示 $0.1500$）。

\textbf{针对问题四}，考虑水分流失导致的尺寸收缩（详见附件2：半径由 $2.0\ \mathrm{cm}$ 收缩至
$1.2\ \mathrm{cm}$），我们首先通过坐标变换，以\textbf{材料（随体）坐标} $\xi=r/R(t)$ 将移动域化为固定域，然后在
$(\xi,z)$ 半域上以节点控制体有限体积离散，水分通量取 \textbf{Kirchhoff 积分均值}（$5$ 点
Gauss 求积）以避免强非线性 $D$ 的假性界面阻力，用隐式 \textbf{BDF} 积分并以事件定位达标时刻，得
$\mathbf{t_{\mathrm{dry}}=51.0908\ \mathrm{h}}$。
...
\textbf{求解思路}：因 $L\gg R$，忽略轴向端部效应与周向不均匀性，将圆柱药材退化为无限长圆柱，问题降维为一维径向问题，仅研究 $r\in[0,R]$。温度场由 Fourier 定律描述，水分场由 Fick
...
温度项，$\rho c_p$ 不能提到时间导数之外，传热与传质双向耦合。
...
\textbf{求解思路}：先利用数据驱动求解出阶段分界时刻$t_c$，然后在三维柱坐标有限体积框架下联立求解两方程，两阶段物性按识别出的
$t_c$ 分段取用并保持切换连续。

\subsection{问题三的分析}
问题三在问题二模型的基础上引入干燥达标判据：药材各处水分浓度均低于
$C_{\mathrm{crit}}=0.15\ \mathrm{kg/kg}$。关键在于如何判断全局水分浓度低于所给阈值

\textbf{求解思路}：在问题二模型上作长时间推进，由于水分浓度沿径向由外向内递增，中截面中心
$(r,z)=(0,L/2)$ 处最高、下降最慢，故“各处均达标”可以转化为“中截面中心 $C(0,L/2,t)<0.15$”，
多维达标判断归结为对单点时间序列的监控，问题转化为临界时间的确定。


\subsection{问题四的分析}
问题四考虑实际烘干中因水分流失导致的尺寸收缩，即求解域外边界 $R(t)$ 随时刻移动，
构成移动边界问题。难点有三：其一，半径收缩使空间离散域逐时刻变化，固定网格难以直接套用；其二，表6 要求输出到
“药材表面”的水分浓度，而该位置本身随时刻移动；其三，$R(t)$ 全程不小于 $1.2\ \mathrm{cm}$，
故“距中心 $1.5\ \mathrm{cm}$”及更远的位置在收缩后已不存在，输出时须留空而不能外推。

\textbf{求解思路}：先采用材料（随体）坐标变换 $\xi=r/R(t)$ 将移动区间 $[0,R(t)]$ 映射为
固定区间 $[0,1]$，把收缩视为材料自身的运动：均匀径向收缩的材料速度为
$v_r=(\dot R/R)\,r$。$R(t)$的连续表达式由附件2 数据作保形分段三次插值得到。然后在考虑材料自身运动的情况下改写传热方程和传质方程。最后采用有限体积格式进行空间离散和隐式BDF方法积分进行数值计算。
...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, draw, rounded corners=1.5pt, align=center,
              inner sep=3.5pt, text width=132mm},
...
\node[pre] (pre) {附件1：烘房温度 $T_a(t)$、空气含湿量 $C_a(t)$（241 点，$\Delta t=60$ s）
  $\;\Rightarrow\;$ Savitzky--Golay 保形滤波（窗宽 31 点、三阶）\\
  $\Rightarrow$ 阶段分界识别：尾段中位数 $+$ MAD 给出稳态温度带，$20$ min 滑动均温与
  升温斜率双条件 $+$ 连续保持 $30$ min };

\node[q1, below=of pre] (a)
  {\textbf{问题一}（预热平衡 $0\sim1800$ s）：降维为无限长圆柱 $\Rightarrow$ 一维径向；
   Fourier 导热 $+$ Fick 扩散，$D=7\!\times\!10^{-9}e^{-0.89/C}$；传热与传质解耦，
   有限体积 $+$ Crank--Nicolson $+$ Picard（$N=1600$，$\Delta t=0.25$ s）};

\node[q2, below=of a] (b)
  {\textbf{问题二}（完整烘干 $0\sim3$ h）：物性分段（$t<t_c$ 用附录2，$t\ge t_c$ 用附录3）
   $\Rightarrow$ \textbf{双向耦合}；三维柱坐标有限体积 $20\!\times\!6\!\times\!32$，面导率调和平均
   $+$ 半网格等效边界导率；后向欧拉 $+$ Picard $+$ 共轭梯度};

\node[q3, below=of b] (c)
  {\textbf{问题三}（达标时刻）：判据“各处 $C<0.15$” $\Leftrightarrow$ 中截面中心达标；二维轴对称
   $(r,z)$，守恒有限体积 $+$ 自适应变阶 BDF（$\mathrm{rtol}=2\times10^{-7}$）；
   长时推进至 $\max C<0.15$，取交叉时刻之后的下一个整秒
  };

\node[q4, below=of c] (d)
  {\textbf{问题四}（考虑尺寸收缩）：材料坐标 $\xi=r/R(t)$，均匀径向收缩、长度不变；
   全程采用附录4 物性；节点控制体有限体积 $+$ \textbf{Kirchhoff 积分均值}界面扩散系数
   $+$ 隐式 BDF 与事件定位 $\Rightarrow t_{\mathrm{dry}}=51.0908$ h};

\node[io, below=of d] (out)
  {输出：表1$\sim$表6 与 result1$\sim$result4.xlsx \quad$\big|$\quad
   数值验证：网格加密、时间步加密、全局水分守恒、判据与参数敏感性};
...
$T(r,t)$ & 药材内部温度（问题一至三） & $^\circ\mathrm{C}$（附录公式用 $\mathrm{K}$） \\
$C(r,t)$ & 药材水分浓度（干基含水率，问题一至三） & $\mathrm{kg/kg}$ \\
...
$J$ & 水分通量：$J_{ij}$ 为节点间通量密度，$J_{\mathrm{surface}}$、$J_{\mathrm{out}}$ 为
边界总通量 & $\mathrm{kg/(m^2\cdot s)}$、$\mathrm{kg/s}$ \\
\midrule
...
$D,\ \widehat D_{ij},\ \Phi$ & 水分扩散系数；相邻节点间的积分平均系数与 Kirchhoff
势函数 $\Phi'(C)=D(C,T)$（问题四） & $\mathrm{m^2/s}$ \\
...
$n_r,\ n_z$ & 问题三、四径向、轴向网格区间数（问题四取半轴向） & — \\
$i,\ j,\ n,\ m$ & 网格节点、时间步与迭代步指标（$n$ 亦表示外法向与窗口样本数） & — \\
$\widetilde T_i,\ C_{ij}$ & 控制体中心处的温度、节点处的水分浓度 & $^\circ\mathrm{C}$、$\mathrm{kg/kg}$ \\
...
与单位矩阵（问题三求解） & — \\
$K_{\max}$ & Picard 迭代次数上限（问题二求解） & — \\
$\mathrm{rtol},\ \mathrm{atol},\ \hat e$ & 求解器相对、绝对容差与局部截断误差估计
（问题三求解） & — \\
$g(t)$ & 达标事件函数 $g(t)=\max_{i,j}C_{ij}(t)-0.15$（问题四求解） & $\mathrm{kg/kg}$ \\
...
经 Savitzky--Golay 平滑后线性插值得到。径向取 $N=1600$ 个区间（节点
$r_i=i\Delta r,\ i=0,\dots,N$，$\Delta r=1.25\times10^{-5}\ \mathrm{m}$），
...
控制体的真实通量与边界面积，本问不再作降维处理，直接在柱坐标 $(r,\varphi,z)$ 下
建立完整三维模型~\cite{ref1}。
...
周期性边界；轴线 $r=0$ 处控制体自然退化，不存在额外的边界面，故无需附加条件。
...
\While{$t<t_{\mathrm{end}}$}
    ...
    \For{$m=1$ \textbf{to} $K_{\max}$} \Comment{Picard 迭代（上限 $K_{\max}$）}
...
$1.5\ \mathrm{h}$ 至 $2.0\ \mathrm{h}$ 间中心温度由 $47.1226\ ^\circ\mathrm{C}$ 升至
$49.0152\ ^\circ\mathrm{C}$，升温速率明显降低，表明药材温度已逐渐接近烘房平台温度。

\textbf{（2）温度场。}温度由表面向中心逐步传递，早期径向温差较大，随后迅速减小。
$0.5\ \mathrm{h}$ 时表面与中心温差为 $3.20\ ^\circ\mathrm{C}$，至 $3.0\ \mathrm{h}$ 仅为
$0.06\ ^\circ\mathrm{C}$；此时中心和侧面温度分别为 $49.8952\ ^\circ\mathrm{C}$ 和
$49.9594\ ^\circ\mathrm{C}$，均已接近 $50\ ^\circ\mathrm{C}$。这说明药材在预热后期已基本热透，
...
$1.9237\ \mathrm{kg/kg}$，侧面仅为 $1.0791\ \mathrm{kg/kg}$，径向差值约为
...
与问题二不同，问题三\textbf{全程采用附录 3 的经验公式}
（式~\eqref{eq:p3rho}--\eqref{eq:p3D}），物性不随阶段切换；环境边界同样取附件 1 的平滑实测
序列（见下节），因此本问的时间推进不含任何由阶段分界触发的参数跳变。

问题二识别出的临界时刻 $t_c=6780\ \mathrm{s}=1.8833\ \mathrm{h}$ 在本问中沿用为
\textbf{阶段说明}：预热平衡阶段（$t<t_c$）与恒温干燥阶段（$t\ge t_c$）的起止仍按该时刻
叙述，便于与问题二的结果对照，但它不参与本问的物性或边界条件计算。
...
J_{\mathrm{surface}}
=
h_m\left(C_a-C_{\mathrm{surface}}\right)dA.
\label{eq:p3boundaryflux}
\end{equation}

其中 $C_{\mathrm{surface}}$ 为表面处的水分浓度，$dA$ 为边界面面积微元，$J_{\mathrm{surface}}$
为全部边界面上的表面传质总量。该格式从离散层面保证全局水分守恒。
...
式~\eqref{eq:p3bdf} 关于 $Z_{n+1}$ 是隐式的，每一时间层用\textbf{修正牛顿迭代}求解。
记残差函数 $G$ 与雅可比矩阵 $\mathcal{J}$

\begin{equation}
G(Z)=\sum_{j=0}^{p}a_{j}Z_{n+1-j}
-\Delta t_{n}\beta_{p}\mathcal{F}(t_{n+1},Z),
\qquad
\mathcal{J}(Z)=\frac{\partial\mathcal{F}}{\partial Z}(t_{n+1},Z),
\label{eq:p3residual}
\end{equation}

则第 $m$ 次迭代的近似解 $Z^{(m)}$ 由下述\textbf{稀疏线性方程组}给出，其中 $I$ 为单位
矩阵、$m$ 为迭代步序号、$\delta Z$ 为牛顿修正量：

\begin{equation}
\left(\frac{a_{0}}{\Delta t_{n}}I-\beta_{p}\mathcal{J}\right)\delta Z
=G\!\left(Z^{(m)}\right),
\qquad
Z^{(m+1)}=Z^{(m)}-\delta Z .
\label{eq:p3newton}
...
由于 $\mathcal{F}$ 只把相邻控制体耦合起来，$\mathcal{J}$ 的非零元仅出现在网格邻接位置。
$n_r=24$、$n_z=50$ 时未知数为 $2\times25\times51=2550$，非零元 $24892$ 个；按该稀疏结构作
有限差分并用稀疏 LU 分解求解式~\eqref{eq:p3newton}。阶数 $p\in[1,5]$ 与步长
$\Delta t_{n}$ 由局部截断误差估计自动选取，要求误差估计 $\hat{e}$ 满足
$\|\hat{e}\|\le\mathrm{rtol}\,\|Z\|+\mathrm{atol}$，其中 $\mathrm{rtol}$、$\mathrm{atol}$
分别为求解器的相对容差与绝对容差；本文取
$\mathrm{rtol}=2.0\times10^{-7}$、$\mathrm{atol}=2.0\times10^{-9}$，并限制
$\Delta t_{n}\le300\ \mathrm{s}$。BDF 的稳定域覆盖左半平面（$p=1,2$ 为 $A$-稳定，
$p=3,4,5$ 为 $A(\alpha)$-稳定），因此步长由精度要求决定，而不受稳定性限制。
...
\begin{equation}
t_{\mathrm{dry}}=56.6517\ \mathrm{h}\quad(=203946\ \mathrm{s}),
\label{eq:p3drytime}
\end{equation}
...
\begin{equation}
C_{\max}=0.1499998\ \mathrm{kg/kg},
\label{eq:p3finalmax}
\end{equation}

全精度值为 $0.1499997659\ \mathrm{kg/kg}<0.15$，余量 $2.4\times10^{-7}$；四位小数输出时
显示为 $0.1500$，是严格不等式判定与格式舍入共同作用的结果。
...
\cmidrule(lr){2-6}
 & 0 & 0.5 & 1.0 & 1.5 & 2.0 \\
\midrule
6.0000  & 1.0170 & 0.9882 & 0.9015 & 0.7550 & 0.5327 \\
12.0000 & 0.4563 & 0.4433 & 0.4032 & 0.3298 & 0.1646 \\
18.0000 & 0.2988 & 0.2915 & 0.2685 & 0.2247 & 0.0853 \\
24.0000 & 0.2374 & 0.2323 & 0.2161 & 0.1842 & 0.0667 \\
30.0000 & 0.2052 & 0.2011 & 0.1882 & 0.1625 & 0.0599 \\
36.0000 & 0.1851 & 0.1817 & 0.1708 & 0.1487 & 0.0566 \\
42.0000 & 0.1712 & 0.1682 & 0.1586 & 0.1390 & 0.0548 \\
48.0000 & 0.1610 & 0.1583 & 0.1496 & 0.1318 & 0.0537 \\
54.0000 & 0.1530 & 0.1505 & 0.1426 & 0.1261 & 0.0529 \\
56.6517 & 0.1500 & 0.1476 & 0.1399 & 0.1240 & 0.0527 \\
\bottomrule
\end{tabularx}
\caption{药材烘干过程中中截面（$z=L/2$）的水分浓度（单位：kg/kg）}
\label{tab:C3}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{第三问药材含水率时空分布与径向差异（达标阈值 $0.15\ \mathrm{kg/kg}$）}
    \label{fig:q3_moisture}
\end{figure}
...
推进。$6\ \mathrm{h}$ 时中心水分浓度仍约为 $1.0170\ \mathrm{kg/kg}$，$54\ \mathrm{h}$ 时
已降至 $0.1530\ \mathrm{kg/kg}$，至 $56.6517\ \mathrm{h}$ 全域达标。

表中给出的是\textbf{中截面 $z=L/2$} 上的径向剖面（与问题一、二、四同一口径）。数值上可以
核实：结束时刻全域最大含水率恰出现在该剖面中心 $r=0$ 处，故表中最后一行
（$t=56.6517\ \mathrm{h}$）的径向最大值即全域最大值，该行数值可直接反映达标判据。最后一行的
$0.1500$ 系四位小数舍入显示，全精度值 $0.1499997659$ 仍严格小于 $0.15\ \mathrm{kg/kg}$。
% =====================================================================
% 问题四
% =====================================================================
...
\label{eq:p4volume}
\end{equation}
所有内界面共用同一数值通量、对相邻控制体以相反符号累加，故内部通量求和精确相消。温度导率取
相邻节点值的\textbf{调和平均}。

水分的界面扩散系数则\textbf{不能}简单取调和平均或算术平均。附录4 的 $D$ 对含水率呈指数型
强非线性（近表面 $C$ 由 $2.55$ 降至 $0.05$ 量级），若用两个节点 $D$ 的调和平均，会把粗网格
表面的低扩散系数错误地扩展到整个网格间距，产生明显的\textbf{假性传质阻力}。为此水分通量取沿
相邻节点状态的\textbf{积分均值}（即 Kirchhoff 变量差分）
\begin{equation}
\widehat D_{ij}=\int_0^1 D\!\left((1-s)C_i+sC_j,\ (1-s)T_i+sT_j\right)\mathrm{d}s,
\qquad
J_{ij}=-\widehat D_{ij}\,\frac{C_j-C_i}{\Delta\ell_{ij}},
\label{eq:p4kirchhoff}
\end{equation}
积分用 $5$ 点 Gauss--Legendre 求积公式。在恒温情形，若 $\Phi'(C)=D(C,T)$，则该通量对应
$J_{ij}=-[\Phi(C_j)-\Phi(C_i)]/\Delta\ell_{ij}$，即 Kirchhoff 变换后的通量差分；当两节点浓度
相等时，积分表达式自然退化到该点的扩散系数，避免了直接差商在相近浓度下的消减误差。该处理不
对结果作任何人为校正，仍是对原扩散方程的通量离散。

主计算采用径向 $96$ 个区间、半轴向 $80$ 个区间（$97\times81$ 个节点）。需要注意，在归一化
...
稀疏 Jacobian 模式。主计算相对容差 $2\times10^{-7}$、绝对容差 $2\times10^{-9}$；考虑前
...
\begin{equation}
t_{\mathrm{dry}}=51.0908\ \mathrm{h}\quad(183927\ \mathrm{s}),
\label{eq:p4drytime}
\end{equation}
此时药材半径为 $1.2000\ \mathrm{cm}$，全场最大含水率为 $0.1499999\ \mathrm{kg/kg}<0.15$；
临界时刻前 $1\ \mathrm{s}$ 的全场最大含水率为 $0.1500005\ \mathrm{kg/kg}$，事件前后方向正确、
量级相符。表~\ref{tab:C4} 给出表6 所需的径向剖面。
...
\midrule
6.0000   & 1.7188 & 1.5370 & 1.0221 & --- & 0.4205 &  \\
12.0000  & 0.7374 & 0.6544 & 0.4082 & --- & 0.1669 & \\
18.0000  & 0.4087 & 0.3686 & 0.2397 & --- & 0.0892 &  \\
24.0000  & 0.2851 & 0.2610 & 0.1790 & --- & 0.0673 &  \\
30.0000  & 0.2263 & 0.2094 & 0.1493 & --- & 0.0595 &  \\
36.0000  & 0.1928 & 0.1796 & 0.1317 & --- & 0.0560 & \\
...
按题目要求，表6 与 result4.xlsx 报告\textbf{中截面}（$z=L/2$）的径向剖面：题目未指定轴向
...
由 $R(6\ \mathrm{h})=1.374\ \mathrm{cm}$ 可知，表6 从第一条记录起 $1.5\ \mathrm{cm}$ 就已落在
...
\caption{问题四中截面水分浓度的时空分布\\（左：热力图，白线为随时间收缩的药材表面，
灰色区域为收缩后已不存在的部分；右：若干典型时刻的径向剖面，$\blacktriangledown$ 为该时刻的
药材表面）}
\label{fig:p4field}
\end{figure}
...
\textbf{（1）烘干时长与前三问的比较。}本问结果为 $51.0908\ \mathrm{h}$，比问题三的
$56.6517\ \mathrm{h}$ 短约 $9.8\%$。但这一差异\textbf{不能全部归因于收缩}：本问同时更换了
扩散系数与热物性公式（附录4 的 $D$ 在相同条件下比附录3 小 $2\sim5$ 倍），两者的作用方向相反。
要分离纯收缩效应，需要保持全部物性与环境一致再做对照计算。直观上，半径收缩到原来的 $59.9\%$
使扩散特征时间 $R^{2}/D$ 缩短到约 $0.36$ 倍，这是收缩使干燥加快的主要机制。

\textbf{（2）表面的快速失水与内部的缓慢干燥。}$6\ \mathrm{h}$ 时药材表面已降至
$0.4205\ \mathrm{kg/kg}$，而中心仍有 $1.7188\ \mathrm{kg/kg}$；表面在 $12\ \mathrm{h}$ 后即降至
$0.15$ 以下（$0.1669\to$ 平台约 $0.0526$，已接近烘房空气含湿量
$C_a=0.049992$）。这说明全过程的\textbf{控速环节始终是内部水分向表面的扩散}。该判断与传质
毕渥数一致：以 $M\approx0.2$、$U=50\ ^\circ\mathrm{C}$ 估计
$D\approx6.2\times10^{-10}\ \mathrm{m^2/s}$，$Bi_m=h_mR/D\approx15\gg1$，即外部传质阻力远小于
内部扩散阻力，表面迅速与环境趋于平衡，而中心只能靠缓慢的扩散逐步降低。

\textbf{（3）中截面中心即全场最湿点。}$51.0908\ \mathrm{h}$ 时，药材中截面中心的含水率为
$0.1499999\ \mathrm{kg/kg}$，恰为整个二维场的最大值；而同一时刻按轴向平均的中心值为
$0.1464\ \mathrm{kg/kg}$。若改用轴向平均值判断达标，会得到\textbf{偏早}的结束时刻。这也从
反面说明：输出表可以用中截面口径，但达标判据必须用完整二维场的最大值。

\textbf{5. 数值验证}

问题四的结束判据是\textbf{全场最大含水率}这一\textbf{点值}，而 $D$ 对含水率呈指数型强非线性，
因此烘干时长对空间离散较敏感，必须单独做收敛检查——问题三在固定尺寸域上的收敛结论\textbf{不能
替代}本问。

\textbf{（1）空间网格收敛性。}固定时间容差，逐级加密网格，结果见表~\ref{tab:p4conv}。
...
\begin{tabular}{lccc}
\toprule
径向 $\times$ 半轴向区间数 & 临界时长/h & 相邻级差/min & 误差缩减比 \\
\midrule
$24\times20$ & $51.11538250$ & ---      & --- \\
$48\times40$ & $51.09577973$ & $-1.176$ & --- \\
$72\times60$ & $51.09206574$ & $-0.223$ & $5.27$ \\
$96\times80$ & $51.09076319$ & $-0.078$ & $2.86$ \\
\bottomrule
\end{tabular}
\caption{问题四空间网格收敛性（$\Delta t$ 容差固定，轴向区间数指半长度方向）}
\label{tab:p4conv}
\end{table}

级差单调递减，末期误差缩减比约 $2.9$；按 Richardson 外推，$96\times80$ 网格的残余误差约为
$2.5\ \mathrm{s}$。因此主文件采用 $96\times80$ 的结果，并在论文中报告“约
$51.1\ \mathrm{h}$”——四位小数只是题目要求的输出格式，并不代表物理预测具有四位小数的精度。

\textbf{（2）时间离散的不敏感性。}在 $48\times40$ 网格上，将相对容差由 $2\times10^{-7}$ 收紧到
$2\times10^{-8}$，并把后期最大内部步长由 $300\ \mathrm{s}$ 降到 $150\ \mathrm{s}$，临界时刻仅变化
$0.000353\ \mathrm{s}$，说明该网格下时间离散误差远小于空间离散误差。

\textbf{（3）守恒与平衡检查。}离散格式满足 $\sum_{ij}V_{ij}\dot C_{ij}+F_{\mathrm{out}}=0$。
由于几何在收缩，这\textbf{不等价于} $\mathrm{d}\!\left(\sum V C\right)/\mathrm{d}t=-F_{\mathrm{out}}$；
干基含水率的干物质加权总量应按参考控制体权重计算（均匀径向收缩时，干物质体积密度与当前控制体
体积之积为常数）。程序检验的是内通量相消与参考干物质权重下的离散平衡，不把 $\rho(C)V$ 当作
恒定干物质质量。此外，均匀场等于环境且无边界差值时，即使 $R(t)$ 变化，含水率也应保持常数——
该检查同样通过；最终瞬时内外通量平衡的相对残差为 $0.0$（机器舍入精度）。

\textbf{（4）环境平滑与物性阶段处理的敏感性。}在 $72\times60$ 网格上，若环境输入不做
Savitzky--Golay 平滑而直接用附件1 原值，临界时长为 $51.0915\ \mathrm{h}$，与平滑方案的
$51.0921\ \mathrm{h}$ 相差约 $2\ \mathrm{s}$，影响可忽略。若沿用问题三的阶段处理方式（在
$4980\sim6780\ \mathrm{s}$ 内用 smoothstep 权重在附录2 与附录4 之间逐项混合 $\rho,c_p,k,D$），
在 $48\times40$ 网格上得到 $50.6860\ \mathrm{h}$，与同网格全程附录4 的 $51.0958\ \mathrm{h}$
相差 $0.4097\ \mathrm{h}$（约 $24.6\ \mathrm{min}$）——\textbf{物性阶段假设造成的差异大于已检验
的时间数值误差}，故本文明确选择题目本问给定的全程附录4 方案。

\textbf{（5）结论与限制。}综合上述检验，本问在给定假设与附录4 物性下给出的烘干时长为
$t_{\mathrm{dry}}\approx51.1\ \mathrm{h}$。数值收敛验证的是给定假设下离散解的稳定性，不能代替
实测干燥曲线对模型的验证；本节所有结论均建立在“均匀径向收缩、长度不变、全程附录4、
忽略蒸发潜热”这组明确假设之上。
...
\begin{itemize}
    ...
    \item \textbf{数值方案稳定且精度可控}：时间方向全部采用隐式格式（Crank--Nicolson、后向欧拉、
    自适应变阶 BDF），对刚性扩散问题无条件稳定，适合 $2\sim3$ 天的长时间推进；问题三、四的
    长时间推进统一为误差可控的自适应 BDF（$\mathrm{rtol}=2\times10^{-7}$），步长由精度而非
    稳定性决定，计算量较固定步长格式下降约两个数量级。
